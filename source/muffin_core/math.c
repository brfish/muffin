...
MufMT19937 *mufCreateMT19937(muf_u32 seed) {
    MufMT19937 *obj = mufAllocate(MufMT19937, 1);
...
muf_bool mufVec2Equal(const muf_rawptr v1, const muf_rawptr v2) {
    const MufVec2 *a = (const MufVec2 *) v1;
    const MufVec2 *b = (const MufVec2 *) v2;
    return a->x == b->x && a->y == b->y;
}

muf_bool mufVec3Equal(const muf_rawptr v1, const muf_rawptr v2) {
    const MufVec3 *a = (const MufVec3 *) v1;
    const MufVec3 *b = (const MufVec3 *) v2;
    return a->x == b->x && a->y == b->y && a->z == b->z;
}

muf_bool mufVec4Equal(const muf_rawptr v1, const muf_rawptr v2) {
    const MufVec4 *a = (const MufVec4 *) v1;
    const MufVec4 *b = (const MufVec4 *) v2;
    return a->x == b->x && a->y == b->y && a->z == b->z && a->w == b->w;
}

muf_index mufVec2Hash(const muf_rawptr v) {
    return mufHash_bytes(v, sizeof(MufVec2));
}

muf_index mufVec3Hash(const muf_rawptr v) {
    return mufHash_bytes(v, sizeof(MufVec3));
}

muf_index mufVec4Hash(const muf_rawptr v) {
    return mufHash_bytes(v, sizeof(MufVec4));
}
...
void mufColorLoad(MufColor *color, muf_f32 *values) {
...
muf_bool mufMat2Equal(const muf_rawptr m1, const muf_rawptr m2) {
...
muf_bool mufMat3Equal(const muf_rawptr m1, const muf_rawptr m2) {
...
muf_bool mufMat4Equal(const muf_rawptr m1, const muf_rawptr m2) {
...
MufMat2 mufMat2Mul(const MufMat2 *m1, const MufMat2 *m2) {
    ...
    return mufCreateMat2(b[0][0] * a[0][0] + b[0][1] * a[1][0], b[0][0] * a[0][1] + b[0][1] * a[1][1],
...
MufMat3 mufMat3Mul(const MufMat3 *m1, const MufMat3 *m2) {
    ...
    return mufCreateMat3(a[0][0] * b[0][0] + a[1][0] * b[0][1] + a[2][0] * b[0][2],
...
MufMat3 mufMat3Scale(const MufMat3 *mat, muf_f32 s) {
    ...
    return mufCreateMat3(m[0][0] * s, m[0][1] * s, m[0][2] * s,
...
MufMat4 mufMat4Scale(const MufMat4 *mat, muf_f32 s) {
    ...
    return mufCreateMat4(m[0][0] * s, m[0][1] * s, m[0][2] * s, m[0][3] * s,
...
MufMat2 mufMat2Inverse(const MufMat2 *mat) {
    muf_f32 fac = 1.0f / mufMat2Determinant(mat);
    const muf_f32 (*m)[2] = mat->values;

    return mufCreateMat2(fac * m[1][1], -fac * m[0][1], -fac * m[1][0], fac * m[0][0]);
}

MufMat3 mufMat3Inverse(const MufMat2 *mat) {
...
MufMat3 mufMat3Transpose(const MufMat3 *mat) {
    ...
    return mufCreateMat3(m[0][0], m[1][0], m[2][0],
        m[0][1], m[1][1], m[2][1],
        m[0][2], m[1][2], m[2][2]
    );
}
...
MufMat4 mufMat4Transpose(const MufMat4 *mat) {
    ...
    return mufCreateMat4(m[0][0], m[1][0], m[2][0], m[3][0],
...
MufMat4 *mufScaleAxis(MufMat4 *base, MufVec3 axis, muf_f32 factor) {
    MufVec3 n = mufVec3Normalize(axis);
    const muf_f32 fx = n.x * (factor - 1);
    const muf_f32 fy = n.y * (factor - 1);
    const muf_f32 fz = n.z * (factor - 1);
